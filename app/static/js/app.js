...
function renderRiskWorkspace(model) {
  ...
  const aiCopy=$('.risk-ai p',root);if(aiCopy)aiCopy.textContent=`${model.product.product_name} is recommended for ${model.line} based on eligible line capability, historical similarity, and predicted startup performance. Overall startup risk is ${overallRisk.toLowerCase()}; the leading watch item is ${primaryRisk.toLowerCase()}. The recommendation uses ${model.referenceCount||5} comparable launches and preserves the modeled uncertainty interval.`;
  const takeaways=$('.risk-takeaways ul',root);if(takeaways)takeaways.innerHTML=`<li>${model.line} is the strongest eligible option for this ${esc(model.product.formula_type)} launch.</li><li>${esc(primaryRisk)} is the primary risk to monitor.</li><li>Historical evidence shows ${model.medianOee||model.oee}% median startup OEE across comparable products.</li><li>The decision reflects ${model.referenceCount||5} line-specific historical references.</li>`;
}
