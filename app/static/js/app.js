...
function renderRisks(data) {
  ...
  renderRiskWorkspace({demo:false,product:data.product,analysisId:data.analysis_id,line:best.line_id,oee:best.predictions.oee,oeeInterval:best.predictions.oee_interval,scrap:best.predictions.scrap_pct,scrapInterval:best.predictions.scrap_interval,throughput:best.predictions.throughput_units_hr,throughputInterval:[Math.round(best.predictions.throughput_units_hr*.91),Math.round(best.predictions.throughput_units_hr*1.09)],positive:positives.length?positives:[["Line Experience",18.2]],negative:negatives.length?negatives:[["Changeover Duration",-7.6]],whatWouldChange:data.what_would_change});
}
...
function loadDemo(){const values={product_name:"New Hydrating Lotion",category:"Skincare",product_family:"Hydration",formula_type:"Lotion",viscosity:14500,fill_volume_ml:200,package_type:"Bottle",container_type:"Plastic Bottle",closure_type:"Pump",filling_technology:"Piston Filling",decoration:"Label",process_complexity:"Medium",previous_product:"Hydrating Serum",previous_product_category:"Skincare",planned_batch_quantity:50000,run_sequence:1,campaign_position:2,days_since_prior_run:4,days_since_last_pm:11,labor_assumption:6};Object.entries(values).forEach(([key,value])=>{const element=$(`[name="${key}"]`);if(element)element.value=value});$("#temporary-sku").value="NPL-SKIN-042";$("[name=compounding_required]").checked=true;renderAnalysisPreview();toast("Sample SKU loaded")}
